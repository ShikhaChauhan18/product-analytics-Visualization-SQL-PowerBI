...
**A one-page Product Analytics Dashboard built for a fictional audio-equipment brand (Asces Sound), covering revenue, profit, and discount performance across countries and customer segments.**

I built this as a hands-on practice project to strengthen my SQL-to-Power-BI workflow — writing the transformation logic in SQL Server, connecting it into Power BI via a native query, and building out the data model, DAX measures, and report from there. I followed a structured project brief to get the core build going, then extended it with year-over-year DAX measures, a discount-band join at the SQL layer, and a dedicated tooltip page — details below in [What I Built vs. Extended](#what-i-built-vs-extended).
...
**The brief from management** was for a single-page Product Analytics Dashboard that could support strategic decision-making, specifically covering:
...
The source data lived across several SQL Server tables — `Product_data`, `Product_sales`, and `discount_data` — which needed to be joined and reshaped before they were analytics-ready. Rather than importing raw tables and modeling relationships entirely inside Power BI, I wrote the transformation logic as a single SQL query and pulled it in through Power Query's **SQL statement (native query)** option — so the join and revenue/profit logic run at the source rather than as a web of Power BI relationships.
...
## 🧩 What I Built vs. Extended

I worked from a structured project brief to build the core dashboard end-to-end — the SQL transformation, the Power BI data model, and the five required visuals. Beyond that base requirement, I added:

- **Year-over-year DAX measures** (`Profit YoY`, `Units Sold YoY`) — built as explicit `CALCULATE`/`VAR` measures against a custom Date Table, rather than relying on Power BI's default date hierarchy, so the comparisons stay accurate regardless of filter context.
- **Discount-band join logic in SQL** — rather than handling discounts as a lookup inside Power BI, I joined a separate `discount_data` table on `discount_band` + `month` directly in the SQL query, so discounted revenue and profit are calculated once, at the source.
- **A dedicated tooltip page** — product-level detail (price, cost, customer-type split) surfaces on hover instead of cluttering the main dashboard, which wasn't part of the original five requirements.
- **Native SQL query in Power Query** instead of importing raw tables and building relationships purely in Power BI — this kept the transformation logic transparent and easy to version as a standalone `.sql` file.

If asked in an interview "did you build this yourself" — this is the honest, accurate answer: yes, I executed the full build myself (SQL, model, DAX, visuals), working from a guided project structure that I then extended in the ways above.
